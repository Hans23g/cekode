...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNyaWpyeHRnanV0ZHBkbnhkeGxiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI2NDU1MDEsImV4cCI6MjA5ODIyMTUwMX0.QyRnC_l-AtqXF8EW3hCCNDV_U2x4mG4VqpSolu-rTNw';
...
const getPlaceholderImage = (name, category = 'Sembako') => {
  const bgColors = {
    'Sembako': '%23f3e8ff', // soft purple
    'Minuman': '%23e0f2fe', // soft blue
    'Snack': '%23fef3c7',   // soft yellow
    'Digital': '%23fee2e2',  // soft red
    'Kosmetik': '%23fce7f3', // soft pink
    'Makanan Instan': '%23dcfce7', // soft green
    'Bumbu Dapur': '%23ffedd5', // soft orange
    'Rumah Tangga': '%23ccfbf1', // soft teal
    'Rokok 18+': '%23f3f4f6', // gray
  };
  const textFill = {
    'Sembako': '%23703d92',
    'Minuman': '%230284c7',
    ...
    'Digital': '%23dc2626',
    'Kosmetik': '%23db2777',
    'Makanan Instan': '%2316a34a',
    'Bumbu Dapur': '%23ea580c',
    'Rumah Tangga': '%230d9488',
    'Rokok 18+': '%234b5563',
  };
  
  const bg = bgColors[category] || '%23f3f4f6';
  const strokeColor = textFill[category] || '%23703d92';
    ...
    <g transform="translate(150, 70)">
      ...
      ${category === 'Minuman' 
        ...
        : `<rect x="30" y="30" width="40" height="50" rx="4" fill="none" stroke="${strokeColor}" stroke-width="6"/>
           <circle cx="50" cy="65" r="8" fill="%23FFD700"/>`
      }
    </g>
    <text x="50%" y="215" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="800" font-size="22" fill="%231F1135">${name}</text>
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 400" width="100%" height="100%">
    <defs>
      <linearGradient id="grad" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" style="stop-color:%23703d92;stop-opacity:1" />
        <stop offset="100%" style="stop-color:%23553C9A;stop-opacity:1" />
      </linearGradient>
    </defs>
    <rect width="1200" height="400" fill="url(%23grad)"/>
    <g opacity="0.1">
      <circle cx="100" cy="100" r="200" fill="%23FFD700" />
      <circle cx="1100" cy="300" r="150" fill="%23FFD700" />
    </g>
    <rect x="30" y="30" width="1140" height="340" rx="25" fill="none" stroke="%23FFD700" stroke-width="4" stroke-dasharray="15 15" opacity="0.3"/>
    <text x="50%" y="170" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="900" font-size="46" fill="%23FFD700">${title}</text>
    <text x="50%" y="240" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="bold" font-size="22" fill="%23FFFFFF" opacity="0.9">Katalog Digital Toko Hana - Pi Ecosystem</text>
    <rect x="525" y="280" width="150" height="45" rx="10" fill="%23FFD700" />
    <text x="600" y="302" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="900" font-size="14" fill="%23703d92">BELI SEKARANG</text>
  </svg>`;
  return `data:image/svg+xml;utf8,${svg.replace(/[\r\n\t]/g, '')}`;
};
...
const getQRISDefaultImage = () => {
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 650" width="100%" height="100%">
    <rect width="500" height="650" rx="30" fill="%23FFFFFF" stroke="%23f3f4f6" stroke-width="4"/>
    <path d="M 0 0 L 500 0 L 500 110 L 0 110 Z" fill="%231a2035"/>
    <rect x="30" y="30" width="130" height="50" rx="8" fill="%23FFFFFF" />
    <text x="95" y="60" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="900" font-size="24" fill="%23dc2626">QRIS</text>
    <text x="250" y="150" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="900" font-size="28" fill="%23703d92">TOKO HANA</text>
    <text x="250" y="180" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="600" font-size="14" fill="%234b5563">NMID: ID1039485762</text>
    <rect x="75" y="210" width="350" height="350" rx="15" fill="%23FFFFFF" stroke="%23703d92" stroke-width="4" filter="drop-shadow(0 4px 10px rgba(0,0,0,0.05))"/>
    <g fill="%231a2035">
      <rect x="100" y="235" width="60" height="60"/>
      <rect x="110" y="245" width="40" height="40" fill="%23FFFFFF"/>
      <rect x="120" y="255" width="20" height="20"/>
      
      <rect x="340" y="235" width="60" height="60"/>
      <rect x="350" y="245" width="40" height="40" fill="%23FFFFFF"/>
      <rect x="360" y="255" width="20" height="20"/>

      <rect x="100" y="475" width="60" height="60"/>
      <rect x="110" y="485" width="40" height="40" fill="%23FFFFFF"/>
    ...
    <rect x="30" y="580" width="440" height="45" rx="10" fill="%23f3f4f6"/>
    <text x="250" y="605" dominant-baseline="middle" text-anchor="middle" font-family="Inter, sans-serif" font-weight="bold" font-size="12" fill="%23703d92">TOKO HANA - SCAN TO PAY</text>
  </svg>`;
  return `data:image/svg+xml;utf8,${svg.replace(/[\r\n\t]/g, '')}`;
};
...
function App() {
  ...
  const handleCheckoutWhatsApp = (e) => {
    ...
    cart.forEach((item, idx) => {
      ...
      message += `${idx + 1}. *${item.product.name}* (x${item.quantity}) = Rp ${itemTotal.toLocaleString('id-ID')}\n`;
    });
    message += `------------------------------------------\n`;
    message += `*Subtotal:* Rp ${cartSubtotal.toLocaleString('id-ID')}\n`;
    message += `*Ongkir:* ${deliveryFee === 0 ? 'Gratis' : `Rp ${deliveryFee.toLocaleString('id-ID')}`}\n`;
    message += `*Total Pembayaran:* *Rp ${cartTotal.toLocaleString('id-ID')}*\n\n`;
  ...
  // Web Speech API Voice Search (V3.5 FINAL)
  ...
  const handleAdminLogin = async (e) => {
    ...
    if (loginPassword === 'hana123') {
      ...
      const { data, error } = await supabaseService.signIn('admin_70666@web-library.net', 'hana123');
      if (error) {
        console.warn('Supabase Auth connection skipped:', error.message);
        showToast('🔐 Login Berhasil (Sesi Cadangan)');
      } else {
        showToast('⚡ Terhubung dengan Supabase Auth!');
      }
      
      setCurrentTab('AdminPage');
      window.location.hash = '#/admin';
    } else {
      alert('Sandi salah! (Sandi: hana123)');
    }
  };
  ...
  const handleAdminLogout = async () => {
    ...
    showToast('🔓 Keluar dari sesi Administrator.');
  };
  ...
  return (
    <div className="flex-1 flex flex-col relative pb-16 bg-white min-h-screen">
      ...
      {currentTab !== 'AdminPage' ? (
        <div className="flex-1 flex flex-col animate-fade-in">
          ...
          <main className="flex-1 overflow-y-auto flex flex-col">
                ...
                {banners.map((banner, index) => (
                  <div 
                    ...
                    className={`absolute inset-0 w-full h-full transition-opacity duration-700 ease-in-out cursor-pointer ${
                      index === activeBannerIndex ? 'opacity-100 z-10 scale-100' : 'opacity-0 z-0 scale-95'
                    }`}
                  >
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 space-y-3 mx-4 bg-[#F5F3F7] rounded-2xl border border-purple-50 shadow-sm">
                <div className="w-10 h-10 border-4 border-purple-200 border-t-[#703d92] rounded-full animate-spin"></div>
                <p className="text-xs font-extrabold text-[#703d92] tracking-wide">Sinkronisasi Supabase Realtime...</p>
              </div>
                      ...
                      <div className="mt-2 flex flex-col justify-end">
                        ...
                        {product.original_price && product.original_price > product.price ? (
                          <div className="flex flex-col">
                            ...
                            <span className="font-extrabold text-red-500 text-sm">Rp {product.price.toLocaleString('id-ID')}</span>
                          </div>
                        ) : (
                          <div className="flex flex-col">
                            <div className="h-3" />
                            <span className="font-extrabold text-[#703d92] text-sm">Rp {product.price.toLocaleString('id-ID')}</span>
                          </div>
                        )}
        ...
        <div className="flex-1 flex flex-col bg-[#F5F3F7] animate-fade-in text-xs">
          ...
          <div className="bg-gradient-to-r from-[#703d92] to-[#553C9A] text-white px-4 py-3 flex items-center justify-between shadow-md flex-shrink-0">
            ...
            <button 
              ...
              className="text-[10px] bg-white/15 hover:bg-white/25 px-3 py-1.5 rounded-xl font-bold flex items-center gap-1"
            >
      ...
      <nav className="fixed bottom-0 left-0 right-0 sm:absolute z-30 bg-white border-t border-gray-100 flex items-end justify-around pb-1.5 pt-2 px-1 shadow-[0_-4px_16px_rgba(0,0,0,0.03)] h-16 flex-shrink-0">
        ...
        <button
          onClick={() => {
            if (adminLoggedIn && currentTab === 'AdminPage') {
              setCurrentTab('Profil');
              ...
              setCurrentTab('Profil');
              setShowLoginModal(true); // Directly pop up admin login modal on profile tab click!
            }
          }}
          className="flex flex-col items-center justify-center flex-1 h-full"
        >
              ...
              {(categories || []).map((cat, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[9px] leading-tight font-extrabold truncate w-full">{cat.name}</span>
                  </button>
                );
              })}
            </div>
      ...
      {showQrisModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowQrisModal(false)}>
          <div className="w-full max-w-sm bg-white rounded-3xl p-5 flex flex-col items-center" onClick={(e) => e.stopPropagation()}>
            <div className="w-full flex items-center justify-between border-b pb-3 mb-4">
              ...
              <button onClick={() => setShowQrisModal(false)} className="p-1 bg-gray-100 text-gray-500 rounded-full"><LucideIcon name="X" size={15} /></button>
            </div>
            <div className="w-full max-w-[280px] aspect-[1/1.3] bg-gray-50 rounded-2xl overflow-hidden flex items-center justify-center">
              <img src={qrisImageUrl || getQRISDefaultImage()} alt="Merchant QRIS" className="w-full h-full object-contain" />
            </div>
            <p className="text-[10px] text-gray-400 font-semibold text-center mt-3 max-w-[250px]">Pindai memakai m-Banking / e-Wallet Anda untuk menyelesaikan pembayaran.</p>
            <button onClick={() => setShowQrisModal(false)} className="w-full mt-4 bg-gradient-to-r from-[#703d92] to-[#553C9A] text-white font-extrabold py-2.5 rounded-xl text-xs shadow-md">
              Selesai Pembayaran
            </button>
          </div>
        </div>
            ...
            <div className="mt-4">
              ...
              <div className="flex justify-between items-center border-t border-dashed mt-3 pt-3">
                <div className="flex flex-col text-xs text-gray-500">
                  <span>Kategori: {selectedProductDetail.category}</span>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <span className="font-black text-lg text-[#703d92]">Rp {selectedProductDetail.price.toLocaleString('id-ID')}</span>
                </div>
              </div>
            </div>
            <button
      ...
      {/* --- ADMIN LOGIN MODAL --- */}
      {showLoginModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowLoginModal(false)}>
          <div className="w-full max-w-sm bg-white rounded-3xl p-6 border animate-slide-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4 border-b pb-3">
              <h4 className="font-extrabold text-sm text-[#703d92] flex items-center gap-1.5">
                <LucideIcon name="Shield" size={16} />
                <span>Login Administrator</span>
              </h4>
              <button onClick={() => setShowLoginModal(false)} className="p-1 text-gray-400 hover:bg-gray-100 rounded-full"><LucideIcon name="X" size={16} /></button>
            </div>
            <form onSubmit={handleAdminLogin} className="space-y-4 text-xs">
              <div className="bg-purple-50 p-3 rounded-xl border">
                <p className="text-[10px] text-purple-700 font-semibold leading-relaxed">Sandi login administrator standar adalah: <strong>hana123</strong></p>
              </div>
              ...
              <button type="submit" className="w-full bg-[#703d92] text-white font-bold py-3 rounded-xl shadow shadow-purple-100 active:scale-95 transition-all">Masuk Panel Admin</button>
            </form>
          </div>
        </div>
      )}
